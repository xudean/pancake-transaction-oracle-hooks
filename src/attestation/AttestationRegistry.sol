...
contract AttestationRegistry is Ownable, IAttestationRegistry {
    ...
    event AttestationSubmitted(
        bytes32 attestationId, address recipient, string exchange, uint256 value, uint256 timestamp
    );
    // fee received event
     ...
     * *
     */
    function setCexCheckListAndJsonPath(string[] memory _dexUrls, string[] memory _dexName, string[] memory _jspnPath)
        external
        onlyOwner
    {
    ...
    function submitAttestation(PrimusAttestation calldata _attestation) public payable returns (bytes32) {
        ...
        bytes32 attestationId = keccak256(
            abi.encodePacked(
                _attestation.recipient, url, exchange, actualParsePath, valueString, _attestation.timestamp
            )
        );
        // save the attestation
        attestations[attestationId] =
            Attestation(attestationId, _attestation.recipient, exchange, uint32(value), _attestation.timestamp);
